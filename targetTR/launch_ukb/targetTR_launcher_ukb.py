...
def RunWorkflow(json_dict, workflow_id, name, job_priority, depends=[]):
    """
...
def main():
    ...
    parser.add_argument("--job-priority", help="use spot or demand instance", required=False, default="normal", type=str)
    parser.add_argument("--batch-size", help="HipSTR batch size", type=int, default=100)
    ...
    parser.add_argument("--file-list", help="List of crams and indices to process"
        "Format of each line: cram-file-id cram-index-id", type=str, required=False, default="ukb_500k_filtered_samples_dragen_cram_and_index_files.txt")
    ...
    parser.add_argument("--max-batches-per-workflow", help="Maximum number of batches to launch at once. -1 means all", required=False, default=20, type=int)
    ...
    log_dir = f"./submission_logs/{args.name}" 
    os.makedirs(log_dir, exist_ok=True)
    print(f"Using samples from {args.file_list}...")
    ...
    if args.max_batches_per_workflow == -1:
        ...
        analysis = RunWorkflow(json_dict, args.workflow_id, args.name, args.job_priority)
    else:
        ...
        while curr_idx < len(cram_batches):
            if len(curr_cram_batches) == args.max_batches_per_workflow:
                ...
                analysis = RunWorkflow(batch_dict, args.workflow_id, \
                    args.name + "/batched/" + args.name+"_%s"%batch_num, args.job_priority, depends=use_dep)
                saveJson(f"{log_dir}/{args.name}_{batch_num}.json", batch_dict)
                with open(f"{log_dir}/{args.name}_analysis_id.txt", "a") as file:
                    file.write(f"{analysis.get_id()}\n")
        ...
        analysis = RunWorkflow(merge_dict, args.merge_workflow_id, f"{args.name}_merged", args.job_priority, depends=depends)
        saveJson(f"{log_dir}/{args.name}_final_merge.json", merge_dict)
