...
gwas_file_s1_pre="${phen}_hail_${s1_samples}_chr${chr}_${from}_${to}"
gwas_file_s2_pre="${phen}_hail_${s2_samples}_chr${chr}_${from}_${to}"
gwasloc="${WORKSPACE_BUCKET}/pipsort/gwas/casecontrol"

if [ -e "$common/${gwas_file_s1_pre}.gwas.tab" ]; then
    echo "no need to copy gwas1"
else
    gsutil cp "${gwasloc}/${gwas_file_s1_pre}.gwas.tab" $common
fi
if [ -e "$common/${gwas_file_s2_pre}.gwas.tab" ]; then
    echo "no need to copy gwas2"
else
    gsutil cp "${gwasloc}/${gwas_file_s2_pre}.gwas.tab" $common
fi
...
$plink_loc/plink2 --bfile ${chr}_${from}_${to}_${phen}_plink --chr $chr --keep plink_$s1_samples_file --make-bed --out s1_data --pheno ${phen}_plink_format.tab --1 --prune --extract union_snps.txt
$plink_loc/plink2 --bfile ${chr}_${from}_${to}_${phen}_plink --chr $chr --keep plink_$s2_samples_file --make-bed --out s2_data --pheno ${phen}_plink_format.tab --1 --prune --extract union_snps.txt
...
python $scripts/get_gwas_in_mscaviar_style.py --infile s1_gwas_temp.txt --outfile s1_gwas.txt --chr_col chrom --pos_col pos --rsid_col rsid --ref_col z_stat --alt_col locus --beta_col beta --se_col standard_error
python $scripts/get_gwas_in_mscaviar_style.py --infile s2_gwas_temp.txt --outfile s2_gwas.txt --chr_col chrom --pos_col pos --rsid_col rsid --ref_col z_stat --alt_col locus --beta_col beta --se_col standard_error
#rm s1_gwas_temp.txt
#rm s2_gwas_temp.txt
...
python $scripts/add_n_col.py $s1_samples_file $s2_samples_file ${phen}_phenocovar.csv s1_orig_gwas s2_orig_gwas
